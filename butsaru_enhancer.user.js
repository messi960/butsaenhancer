...
} else if ( GM_getValue.toString && GM_getValue.toString().indexOf("not supported")>-1 ) {
...
} else if ( GM_setValue.toString && GM_setValue.toString().indexOf("not supported")>-1 ) {
...
beScript.organizer = {
    ...
    addLastMatchesResults : function() {
        ...
        if ( teamOptions.length > 0 ) {
            ...
            {
                _teams[i] = {};
                _teams[i].name = teamOptions[i].innerHTML;
                _teams[i].id = teamOptions[i].value;

                var t = GM_getValue( beScript.NAMESPACE + ".organizer.team." + _teams[i].id);
            ...
            beScript.organizer.teams = _teams;
        }
    },
/*    addRepairBuildingsLink : function() {
        var _teams = beScript.organizer.teams;
        for ( var i = 0; i < _teams.length; i++ ) {
            var teamtablerow = document.evaluate('//td[contains(.,"' + _teams[i].name + '") and parent::tr[@bgcolor="#ffffff" or @bgcolor="#EEF4FA"]]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null ).singleNodeValue.parentNode;
            var teamBuildings = teamtablerow.childNodes[16].textContent;
            var conds = beScript.Util.checkByRegExp( teamBuildings, /([^\s]+)\s\/\s([^$]+)/ );
            if ( parseFloat(conds[1]) < 100 ) {
                beScript.log( conds );
            }
        }
    },*/
...
//            beScript.organizer.addRepairBuildingsLink();
        }
    }
};
