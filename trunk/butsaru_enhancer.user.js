...
beScript.log( player );
        ...
        player.number = parseInt( beScript.Util.checkByRegExp(playerInfo.text().trim(), /^\((\d+)\)/ ) );
        player.name = $( "a[href*='xml/players']", playerInfo ).text().trim();
        player.country = {};
        player.country.name = $( "img", playerInfo ).attr( "title" ).trim();
        player.country.id = parseInt(beScript.Util.checkByRegExp( $( "img", playerInfo ).attr( "src" ), /(\d+)\.gif/ )[1]);
    ...
    loadTeamPlayers : function( team, force ) {
        ...
        if ( !playersLoaded || timeUpdaterFired || force ) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    beScript.log( playersRows.size() + " players in " + team.name );
                    var counter = 0;

                    playersRows.each(function(i) {
                        var fields = $( "td", $(this) );
                        var fields = $( "td", playersRows.eq(0) );
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                beScript.log( counter + " players handled in " + team.name );
                            }
                        });
                    });
...
//        beScript.log( season );
    ...
    process : function() {
        ...
        beScript.parsePlayerInfo(player, $("body"));
